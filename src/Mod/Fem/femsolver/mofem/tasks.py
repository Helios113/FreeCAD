...
class Results(run.Results):
    ...
    #TODO
    #Create a new parent object which will hold all VTK files
    #It will be able to scroll through all of them
    #And provide the tools to change their appearance
    #Kinda like the FC result object with a slider
    #This had to work by effectively removing all other objects with scroll
    def run(self):
        ...
        if self.solver.MoFEMResult is None:
            self._createResults(res_dir)

    def _createResults(self, dir):
        importVTKResults.importVtkVtkResult(dir, self.solver.Name + "Result")
        importVTKResults.importVtkFCResult(dir, self.solver.Name + "Result", self.analysis)
